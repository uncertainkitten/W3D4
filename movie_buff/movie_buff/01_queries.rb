...
def harrison_ford
  ...
  Movie.select('movies.id, movies.title').joins(:actors).where('actors.name = ? AND castings.ord != ?', 'Harrison Ford', 1)
end
...
def biggest_cast
  ...
  Movie.select('movies.id, movies.title, COUNT(movies.id)')
  .joins(:actors)
  .group('movies.id')
  .order('COUNT(movies.id) DESC')
  .limit(3)
